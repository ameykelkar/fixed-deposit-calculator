...
def calculate_next_interest_date(start_date, frequency, today, maturity_date):
    ...
    maturity_ts = pd.Timestamp(maturity_date) if not isinstance(maturity_date, pd.Timestamp) else maturity_date
...
def calculate_financial_year_interest_dates(start_date, frequency, fy_start, fy_end, maturity_date):
    if frequency not in ['H', 'Y', 'Q', 'M', 'C']:
        return []
    ...
    while current_date <= fy_end_ts:
        ...
        if current_date > maturity_date_ts:
            ...
            if maturity_date_ts >= fy_start_ts and maturity_date_ts <= fy_end_ts and maturity_date_ts not in interest_dates:
...
def main():
    ...
    # Read the Excel file
    try:
        df = pd.read_excel('data/data.xlsx', sheet_name='vivek fd 2025')
        ...
        df['FY_INTEREST_AMOUNT'] = df.apply(
            lambda row: len(row['FY_INTEREST_DATES']) * calculate_interest_amount(
                row['DEPOSIT AMT'], 
                row['RATE OF INT.'], 
                row['INTEREST PAYABLE']
            ) if row['INTEREST PAYABLE'] != 'C' else calculate_interest_amount(
                row['DEPOSIT AMT'], 
                row['RATE OF INT.'], 
                row['INTEREST PAYABLE']
            ) if any(date <= fy_end for date in row['FY_INTEREST_DATES']) else 0,
            axis=1
        )
        
        # Create a column to flag deposits with interest due this month
        df['DUE THIS MONTH'] = df['NEXT INTEREST DATE'].apply(
            lambda x: pd.notnull(x) and pd.Timestamp(x).month == today.month and pd.Timestamp(x).year == today.year
        )
        
        # Format currency columns for display
        display_df = df.copy()
        display_df['DEPOSIT AMT'] = display_df['DEPOSIT AMT'].apply(format_currency)
        display_df['INTEREST AMOUNT'] = display_df['INTEREST AMOUNT'].apply(format_currency)
        display_df['RATE OF INT.'] = display_df['RATE OF INT.'].apply(lambda x: f"{x:.2%}")
        ...
        display_df['INTEREST PAYABLE'] = display_df['INTEREST PAYABLE'].map(frequency_map)
        
        # Display all deposits
        st.header("All Fixed Deposits")
        st.dataframe(
            display_df[['DEP NO', 'NAME OF THE DEPOSITEE', 'DATE', 'MATURITY DATE', 
                     'DEPOSIT AMT', 'RATE OF INT.', 'INTEREST PAYABLE', 
                     'NEXT INTEREST DATE', 'INTEREST AMOUNT']].astype(str),
            hide_index=True,
        ...
        if not this_month_df.empty:
            st.dataframe(
                display_this_month_df[['DEP NO', 'NAME OF THE DEPOSITEE', 'DATE', 
                                      'DEPOSIT AMT', 'RATE OF INT.', 'INTEREST PAYABLE', 
                                      'NEXT INTEREST DATE', 'INTEREST AMOUNT']].astype(str),
                hide_index=True,
                use_container_width=True
            ...
            st.success(f"Total interest to be received in {today.strftime('%B %Y')}: {format_currency(total_interest)}")
        else:
            st.info(f"No deposits will pay interest in {today.strftime('%B %Y')}")
        
        # Display financial year interest summary
        st.markdown("---")
        st.header(f"Financial Year Interest Summary ({fy_start.strftime('%b %d, %Y')} to {fy_end.strftime('%b %d, %Y')})")
        ...
        if not fy_df.empty:
            ...
            fy_display_df['DEPOSIT AMT'] = fy_display_df['DEPOSIT AMT'].apply(format_currency)
            fy_display_df['FY_INTEREST_AMOUNT'] = fy_display_df['FY_INTEREST_AMOUNT'].apply(format_currency)
            fy_display_df['RATE OF INT.'] = fy_display_df['RATE OF INT.'].apply(lambda x: f"{x:.2%}")
            fy_display_df['INTEREST PAYABLE'] = fy_display_df['INTEREST PAYABLE'].map(frequency_map)
            fy_display_df['INTEREST FREQUENCY'] = fy_display_df['FY_INTEREST_DATES'].apply(lambda x: f"{len(x)} payment(s)")
            
            # Show the table
            st.dataframe(
                fy_display_df[['DEP NO', 'NAME OF THE DEPOSITEE', 'DEPOSIT AMT', 
                              'RATE OF INT.', 'INTEREST PAYABLE', 'INTEREST FREQUENCY', 'FY_INTEREST_AMOUNT']].astype(str),
                hide_index=True,
                use_container_width=True
            )
            ...
            st.success(f"Total interest earned in financial year {fy_start.year}-{fy_end.year}: {format_currency(total_fy_interest)}")
        else:
            st.info(f"No interest earned in financial year {fy_start.year}-{fy_end.year}")
